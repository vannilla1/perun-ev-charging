...
const MapIcon = ({ filled = false }) => (
  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 sm:h-6 sm:w-6" fill={filled ? "currentColor" : "none"} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={filled ? 0 : 1.75}>
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 safe-area-bottom">
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(90deg, transparent 0%, rgba(0, 212, 255, 0.4) 30%, rgba(0, 255, 136, 0.4) 70%, transparent 100%)',
        }}
      />
          ...
          return (
            <Link
              ...
              className={`
                ...
                transition-all duration-250 rounded-xl sm:rounded-2xl
                min-w-[56px] sm:min-w-[64px]
                active:scale-95
                ${active
                  ? 'text-[#00D4FF]'
                  : 'text-[#3D5A7A] hover:text-[#7A9CC0]'
                }
              `}
            >
              ...
              {/* Icon */}
              <div className={`relative z-10 transition-all duration-250 ${active ? 'scale-110' : ''}`}>
              ...
              <span
                className={`
                  relative z-10 text-[10px] sm:text-[11px] tracking-wide
                  transition-all duration-250
                  ${active ? 'font-semibold' : 'font-medium'}
                `}
                style={active ? { fontFamily: "'Space Mono', monospace", letterSpacing: '0.02em' } : {}}
              >
                {t(item.id)}
              </span>
